...
function Gallery() {
  ...
  useEffect(() => {
    fetch(`${BASE_URL}/api/photos/categories`)
      .then((res) => res.json())
  ...
  useEffect(() => {
    ...
    const url = selectedCategory
      ? `${BASE_URL}/api/photos/category/${selectedCategory}?page=${page}&limit=${IMAGES_PER_LOAD}`
      : `${BASE_URL}/api/photos?page=${page}&limit=${IMAGES_PER_LOAD}`;
    ...
    fetch(url)
      ...
      .then((data) => {
        ...
        setHasMore(newPhotos.length === IMAGES_PER_LOAD);
        setLoading(false);
      })
      .catch((err) => {
  ...
  return (
    <section className="gallery-section" id="gallery">
      ...
      >
        <button
          className={`gallery-btn ${selectedCategory === "" ? "active" : ""}`}
      ...
      <div className="gallery-grid">
        {images.length > 0 ? (
          ...
          <p className="no-photos-message" data-aos="fade-up">
